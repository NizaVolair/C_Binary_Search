 ...
 ** Author: Niza Volair
 ** Date: 07-26-15
 ** Description: This is the implementation file for the binary search tree data structure
 ** Input: None
...
{
	/* Base case, add new node, return node to recursively rebuilt tree */
	    if(cur == NULL){
        struct Node *newnode;
    ...
    /* Recursive cases, look right or left down the tree making a recursive call at each step */
    if(compare(val, cur->val) == -1)
        cur->left = _addNode(cur->left, val);
    else if(compare(val, cur->val) == 1)
        cur->right = _addNode(cur->right, val);

    /* Returns each node that has been looked through to recursively rebuild the tree */
    return cur;
}
...
{
	/* Make a temp node pointer to go through tree*/
    struct Node *cur = tree->root;
	while (cur != 0)
    {	if (compare(val,cur->val) == 0) //Value found return true
			return 1;
		if (compare(val, cur->val) == -1) //Continue to look right for smaller value, left for bigger
			cur = cur->left;
		else cur = cur->right;
	}
...
{
	/* Go down the tree from current to the leftmost node and return that value */
	assert(cur != 0);
    while (cur->left != 0)
    {   cur = cur->left;
	}
...
/*
 ...
 function returns cur if its left child is NOT NULL. Otherwise,
...
{
    /* Recursively step down list to the left looking for leftmost and returning current to rebuild */
  	if (cur->left != 0)
    {  	cur->left = _removeLeftMost(cur->left);
        return cur;
	}

    /* Return the right child of cur and free cur */
	struct Node *temp;
	temp = cur->right;
	free(cur);
	return temp;
...
/*
 ...
 have to use the compare() function to compare values.
 param:	cur	the current node
...
{
	/* assert val is in the tree, cur is not null, val is not null*/
	assert(cur != NULL);
	assert(val != NULL);

	/* Base case, val is the one being searched for */
    if(compare(val, cur->val) == 0)
    {   /* Right child is NULL, so use a temp to replace current with left child, and free current */
        if (cur->right == 0)
        {   struct Node *temp;
            temp = cur->left;
			free(cur);
    	...
    	/* Replace the value with the leftmost child of the right child
            recursively rebuild from that point */
        cur->val = _leftMost(cur->right);
        cur->right = _removeLeftMost(cur->right);
    }
	/* If value is smaller check to left, if bigger go right */
    else if (compare(val, cur->val) == -1)
        cur->left = _removeNode(cur->left, val);
    else
        cur->right = _removeNode(cur->right, val);

    /* Return the current node each time so that the tree will rebuild on last call */
    return cur;
}
/*
